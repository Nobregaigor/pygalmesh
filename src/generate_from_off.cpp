...
namespace pygalmesh
{
      ...
      const double feature_angle)
  {
    ...
    if (feature_angle > 0)
    {
      if (max_edge_size_at_feature_edges == 0)
      {
        std::stringstream msg;
        msg << "Feature detection requires a non-zero value for max_edge_size_at_feature_edges" << std::endl;
        msg << "Please provide a non-zero value for max_edge_size_at_feature_edges" << std::endl;
        msg << "or set feature_angle to 0 to disable feature detection" << std::endl;
        msg << "max_edge_size_at_feature_edges received: " << max_edge_size_at_feature_edges << std::endl;
        throw std::runtime_error(msg.str());
      }
      cgal_domain.detect_features(feature_angle);
    }
